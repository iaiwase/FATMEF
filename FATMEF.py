...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        # x = x.transpose(1, 2) # 1 1024 144
        x = x.view(B, 1, x_size[0], x_size[1])  # B Ph*Pw C
        return x
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.patch_embed(x)  # 1 9216 128 96*96
        if self.ape:
        ...
        # x = [1 144 1024]
        # x = self.norm(x)  # 1 144 1024 B L C
        # x = self.avgpool(x.transpose(1, 2))  # 1 1024 1 B C 1
...
class DFF(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(2, 2, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x_real = x_fft.real
        x_imag = x_fft.imag
        fft_x = torch.cat((x_real, x_imag), dim=1) # [B 2 H W/2]
        x = self.conv1(fft_x)
        x = self.bn(x)
        x = self.relu(x)
        residual = x
        ...
        imaginary_part = x[:,1,:,:]
        real_part = x[:,0,:,:]
        x = torch.complex(real_part,imaginary_part).unsqueeze(1)
...
class Encoder_Trans(nn.Module):
    ...
    def __init__(self):
        ...
        self.inc1 = Bottle2neck(1, 4)
        self.inc2 = Bottle2neck(16, 4)
        self.inc3 = Bottle2neck(16, 4)
    ...
    def forward(self, x):
        ...
        x_e = self.inc1(x)
        x_e = self.inc2(x_e)
        x_e = self.inc3(x_e) + residual
        
        x = Resize([224,224])(x)
        x = self.dff(x)
        x_t = torch.cat((x, x, x), dim=1)
        x_t = self.transformer(x_t)
        x_t = Resize([256,256])(x_t)
        x_t = self.conv(x_t)
